...
export class AssemblyVM {
  ...
  private originalLines: string[] = []; // 保存原始代码行，用于显示
  ...
  loadAssembly(assembly: string): void {
    ...
    this.labels.clear();
    this.output = [];
    this.originalLines = assembly.split('\n');
    this.resetState();
    ...
    // 查找 main 标签并设置 PC 到 main 的入口地址
    const mainAddress = this.labels.get('main');
    if (mainAddress !== undefined) {
      this.state.pc = mainAddress;
    } else {
      // 如果没有 main 标签，从地址 0 开始（不应该发生，因为编译器已经检查了）
      console.warn(`[VM] 警告：未找到 main 标签，从地址 0 开始执行`);
      this.state.pc = 0;
    }
  }
  ...
  private call(label: string): void {
    // 将返回地址压栈
    // 注意：由于 call 会修改 PC，step() 不会自动递增 PC
    // 所以返回地址应该是当前 PC + 1（下一条指令的地址）
    const returnAddress = this.state.pc + 1;
    const spValue = this.state.registers.get('sp') || 0;
    const targetAddr = spValue - 1;
    
    // 将返回地址压栈（栈向下增长，所以 sp - 1）
    this.state.stack.set(targetAddr, returnAddress);
    this.state.registers.set('sp', targetAddr);
  ...
  private ret(): void {
    // 从栈中弹出返回地址
    const spValue = this.state.registers.get('sp') || 0;
    const returnAddress = this.state.stack.get(spValue); // 不使用 || 0，保持 undefined
    
    // 如果栈中没有有效的返回地址（undefined），说明是 main 函数返回，结束程序
    ...
    this.state.stack.delete(spValue);
  }
  ...
  private push(operand: string): void {
    ...
    this.state.stack.set(targetAddr, value);
    this.state.registers.set('sp', targetAddr);
  }
  ...
  private pop(operand: string): void {
    ...
    const value = this.state.stack.get(spValue);
    
    if (operand === 'ebp' && spValue !== bpValue) {
      console.warn(`[POP] ⚠️  Warning: pop ebp but SP (${spValue}) != BP (${bpValue})`);
    }
    
    // 如果栈中没有值，使用 0（但应该记录警告）
    const finalValue = value !== undefined ? value : 0;
    if (value === undefined) {
      console.warn(`[POP] ⚠️  Warning: stack[${spValue}] is undefined, using 0`);
      if (operand === 'ebp') {
        console.warn(`[POP] ⚠️  Critical: Cannot restore BP! Expected value at stack[${spValue}] but found undefined`);
      }
    }
    
    this.state.registers.set('sp', spValue + 1);
    this.setValue(operand, finalValue);
  }
  ...
  reset(): void {
    ...
    if (mainAddress !== undefined) {
      ...
      console.warn(`[VM Reset] 警告：未找到 main 标签，从地址 0 开始执行`);
      this.state.pc = 0;
    }
    this.state.halted = false;
  }
