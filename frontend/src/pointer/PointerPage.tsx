...
int main() {
    ...
    int *p;
    p = &a;
    *p = 123;  // 通过指针修改 a，此时 a = 123
    add(p, 100);  // 调用 add，通过指针修改 a：*p = a + *p = 100 + 123 = 223
    return a;  // 应该返回 223
}
`
  },
  { 
    label: '二级指针', 
    value: `int modify(int **pp, int value) {
  **pp = value;  // 通过二级指针修改值
  return;
}
...
int main() {
    ...
    **pp = 10;   // 通过二级指针修改 a：a = 10
    
    modify(pp, 100);  // 调用 modify，通过二级指针修改 a：a = 100
...
`
  ...
  { 
    ...
    *p1 = 2;                    // 通过一级指针修改 a：a = 2
    **p2 = *p1 + 3;            // 通过二级指针修改 a：a = 2 + 3 = 5
    ***p3 = **p2 + 4;           // 通过三级指针修改 a：a = 5 + 4 = 9
...
const PointerPage: React.FC = () => {
  ...
  const expressionToString = useCallback((expr: any): string => {
    ...
    if (exprType === 'BinaryExpression') {
      const left = expr.left ? expressionToString(expr.left) : '?';
      const right = expr.right ? expressionToString(expr.right) : '?';
      const op = expr.operator || '?';
      return `(${left} ${op} ${right})`;
    }
    
    if (exprType === 'UnaryExpression') {
      const operand = expr.operand ? expressionToString(expr.operand) : '?';
      const op = expr.operator || '?';
      return `${op}${operand}`;
    }
    ...
    if (exprType === 'ParenthesizedExpression') {
      const inner = expr.expression ? expressionToString(expr.expression) : '?';
      return `(${inner})`;
    }
    
    if (exprType === 'AddressOfExpression') {
      const operand = expr.operand ? expressionToString(expr.operand) : '?';
      return `&${operand}`;
    }
    
    if (exprType === 'DereferenceExpression') {
      const operand = expr.operand ? expressionToString(expr.operand) : '?';
      return `*${operand}`;
    }
  ...
  const statementToString = useCallback((stmt: any): string => {
    ...
    if (stmtType === 'AssignmentStatement') {
      // target 可能是 Identifier 或 DereferenceExpression
      let target: string = 'unknown';
      if (!stmt.target) {
        // target 为空，使用默认值
      } else {
        const targetType = String(stmt.target.type || ''); // 确保是字符串
        if (targetType === 'Identifier') {
          target = stmt.target.name || 'unknown';
        } else if (targetType === 'DereferenceExpression') {
          // 对于解引用赋值 *p = value，需要递归处理多级解引用
          let derefLevel = 0;
          let currentTarget: any = stmt.target;
          while (currentTarget && String(currentTarget.type) === 'DereferenceExpression') {
            derefLevel++;
            currentTarget = currentTarget.operand;
          }
          // 处理 baseTarget：如果是 Identifier，直接使用 name；否则使用 expressionToString
          let baseTarget: string;
          if (currentTarget && String(currentTarget.type) === 'Identifier') {
            baseTarget = currentTarget.name || '?';
          } else if (currentTarget) {
            baseTarget = expressionToString(currentTarget);
          } else {
            baseTarget = '?';
          }
          target = '*'.repeat(derefLevel) + baseTarget;
        } else {
          // 尝试使用 expressionToString 处理
          const exprStr = expressionToString(stmt.target);
          target = exprStr || 'unknown';
        }
      }
      const value = stmt.value ? expressionToString(stmt.value) : '?';
      return `${target} = ${value};`;
    }
  ...
  const initializeCfgExecution = useCallback((targetCfg: ControlFlowGraph, preserveAutoExecuting: boolean = false) => {
    ...
    // 设置当前 CFG 和函数名（用于 handleFunctionParameters 和 generateReturn）
    newAssemblyGenerator.currentCfg = targetCfg;
    (newAssemblyGenerator as any).currentFunctionName = targetCfg.functionName;
  ...
  const handleNextStep = useCallback(() => {
    ...
    if (currentStepIndex < flattenedStatements.length) {
      ...
      if (stmtType === 'VariableDeclaration' || stmtType === 'LetDeclaration') {
        ...
        // 对于赋值语句，target 可能是 Identifier 或 DereferenceExpression
        if (nextStmt.target?.type === 'Identifier') {
          variableName = nextStmt.target.name || null;
        } else if (nextStmt.target?.type === 'DereferenceExpression') {
          // 对于解引用赋值，从 operand 中提取变量名
          let currentTarget = nextStmt.target;
          while (currentTarget?.type === 'DereferenceExpression') {
            currentTarget = currentTarget.operand;
          }
          if (currentTarget?.type === 'Identifier') {
            variableName = currentTarget.name || null;
          }
        }
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 px-6 py-4 pl-20 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-6">
            <h1 className="text-2xl font-bold text-gray-900">指针操作与内存管理</h1>
