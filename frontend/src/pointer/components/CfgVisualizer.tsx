...
function expressionToSourceCode(expr: Expression): string {
  ...
  switch (expr.type) {
    ...
    case 'AddressOfExpression':
      const addrExpr = expr as any;
      const addrOperand = expressionToSourceCode(addrExpr.operand);
      return `&${addrOperand}`;
    case 'DereferenceExpression':
      const derefExpr = expr as any;
      // 处理多级解引用
      let derefLevel = 0;
      let currentDeref = derefExpr;
      while (currentDeref && currentDeref.type === 'DereferenceExpression') {
        derefLevel++;
        currentDeref = currentDeref.operand;
      }
      const derefOperand = currentDeref ? expressionToSourceCode(currentDeref) : '?';
      return '*'.repeat(derefLevel) + derefOperand;
    case 'ParenthesizedExpression':
      const parenExpr = expr as any;
      return `(${expressionToSourceCode(parenExpr.expression)})`;
    default:
      return '...';
  }
}
  ...
  switch (stmtType) {
    ...
    case StatementType.ASSIGNMENT_STATEMENT: {
      ...
      if (!assignStmt.target) {
        targetStr = 'unknown';
      } else if (assignStmt.target.type === 'Identifier') {
        targetStr = assignStmt.target.name || 'unknown';
      } else if (assignStmt.target.type === 'DereferenceExpression') {
        // 对于解引用赋值 *p = value，需要递归处理多级解引用
        let derefLevel = 0;
        let currentTarget = assignStmt.target;
        while (currentTarget && currentTarget.type === 'DereferenceExpression') {
          derefLevel++;
          currentTarget = currentTarget.operand;
        }
        // 处理 baseTarget：如果是 Identifier，直接使用 name；否则使用 expressionToSourceCode
        let baseTarget: string;
        if (currentTarget && currentTarget.type === 'Identifier') {
          baseTarget = currentTarget.name || '?';
        } else if (currentTarget) {
          baseTarget = expressionToSourceCode(currentTarget);
        } else {
          baseTarget = '?';
        }
        targetStr = '*'.repeat(derefLevel) + baseTarget;
      } else {
        // 尝试使用 expressionToSourceCode 处理
        targetStr = expressionToSourceCode(assignStmt.target) || 'unknown';
      }
      return `${targetStr} = ${expressionToSourceCode(assignStmt.value)};`;
    }
